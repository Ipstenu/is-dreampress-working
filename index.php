		...
		<?php
			...
			if ( isset($_SESSION['last_submit']) && time()-$_SESSION['last_submit'] < 30 ) {
				...
				if ( StrictUrlValidator::validate( $varnish_url, true, true ) === false ) {
					...
					$not_cachable     = ( ( isset( $varnish_headers['X-Cacheable'] ) && strpos( $varnish_headers['X-Cacheable'] ,'NO') !== false ) || ( isset( $varnish_headers['Pragma'] ) && strpos( $varnish_headers['Pragma'] ,'no-cache') !== false ) || !$x_age )? true : false;
